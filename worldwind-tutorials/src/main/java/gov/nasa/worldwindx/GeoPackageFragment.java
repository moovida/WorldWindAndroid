...
import gov.nasa.worldwindx.mapsforge.Mapsforge;
import gov.nasa.worldwindx.mapsforge.MapsforgeFactory;
...
public class GeoPackageFragment extends BasicGlobeFragment {
    ...
    public WorldWindow createWorldWindow() {
        Mapsforge.createInstance(getActivity().getApplication());
...
//        );
        ...
        mapsforgeFactory.createFromMapsforge(
                mapsforgePath, // file path on the local Android filesystem
                new MapsforgeFactory.Callback() {
                    @Override
                    public void creationSucceeded(MapsforgeFactory factory, Layer layer) {
                        // Add the finished GeoPackage layer to the World Window.
                        getWorldWindow().getLayers().addLayer(layer);
                    ...
                    @Override
                    public void creationFailed(MapsforgeFactory factory, Layer layer, Throwable ex) {
                        // Something went wrong reading the Mapsforge.
                        Log.e("gov.nasa.worldwind", "Mapsforge layer creation failed", ex);
                    }
                }
        );
